...
void Task500ms(void)
{
	if(bFlag_Task500ms)
	{
		bFlag_Task500ms = false;

		blestate = P2P_Client_APP_Get_State();
		if(P2P_Client_APP_Get_State() != APP_BLE_CONNECTED_CLIENT)	HAL_GPIO_TogglePin(LED_SW_GPIO_Port, LED_SW_Pin);
		else	HAL_GPIO_WritePin(LED_SW_GPIO_Port, LED_SW_Pin, GPIO_PIN_SET);
	}
}
